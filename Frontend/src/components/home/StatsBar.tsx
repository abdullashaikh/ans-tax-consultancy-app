...
export const StatsBar: React.FC = () => {
  return (
    <section className="py-14 sm:py-20 bg-[#0c1833] text-white relative overflow-hidden shadow-md">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
          {(statsData as StatItem[]).map((stat, idx) => (
            <motion.div
              ...
              className={`flex flex-col items-center text-center p-4 sm:p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-amber-400/60 hover:bg-white/10 transition-all duration-300 group hover:-translate-y-1 ${
                idx === statsData.length - 1 && statsData.length % 2 !== 0 ? 'col-span-2 md:col-span-1' : ''
              }`}
            >
              <div className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-amber-400 group-hover:text-amber-300 transition-colors">
                <AnimatedCounter
                  value={stat.value}
                  suffix={stat.suffix}
